...
# Replay Buffer
from Experiments_Engine.Function_Approximators import nStep_QLearning_ExperienceReplay
...
from Experiments_Engine.RL_Agents import ExperienceReplay_Agent, nStep_QLearning_ReturnFunction
# Policy
...
class ExperimentAgent():
    ...
    def __init__(self, experiment_parameters, restore=False, restore_data_dir=""):
        ...
        if restore:
            ...
            " QSigma Agent and Return Function "
        ...
        self.rl_return_fun = nStep_QLearning_ReturnFunction(config=self.config)

        """ QSigma Replay Buffer """
        self.erp = nStep_QLearning_ExperienceReplay(config=self.config, return_function=self.rl_return_fun)

        """ Neural Network """
        self.function_approximator = NeuralNetwork_wER_FA(optimizer=self.optimizer, target_network=self.tnetwork,
                                                          update_network=self.unetwork, er_buffer=self.erp,
        ...
        self.agent = ExperienceReplay_Agent(function_approximator=self.function_approximator,
                            ...
                            er_buffer=self.erp, config=self.config, summary=self.summary)
    ...
    def save_parameters(self, dir_name):
        ...
        params_txt.write("# Agent: n-Step Q-Learning #\n")
