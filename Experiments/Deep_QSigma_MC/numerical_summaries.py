...
def get_agents_data(results_path, method_name, sample_size=None):
    """ Returns the data for all the agents corresponding to one method """
    method_dir = os.path.join(results_path, method_name)
    # Sample size is only to verify that the umber of agents is correct
    if sample_size is not None:
        assert sample_size == len(os.listdir(method_dir))

    return_data = []
    for agent in os.listdir(method_dir):
        agent_path = os.path.join(method_dir, agent)
...
def results_summary_data(pathname, evaluation_episodes, average_window, omit_list=[], ci_error=0.05,
                         name="", fa_windows=[50,500], drop_k=0, max_sample_size=100):
    addtofile = False
    for dir_name in os.listdir(pathname):
        if dir_name in omit_list:
            ...
            if os.path.isdir(dir_path):
                ...
                headers = ["Method Name", "Evaluation Episode", "Average", "Standard Deviation", "Lower C.I. Bound",
                           "Upper C.I. Bound", "Margin of Error"]
                columns = [method_name, evaluation_episodes, np.round(average, 2), np.round(std, 2),
...
if __name__ == "__main__":
    ...
    if args.numerical_summaries:
        ...
        evaluation_episodes = np.arange(50, 550, 50)
        average_window = 50
        omit_list = []
        results_summary_data(results_path, evaluation_episodes, average_window, ci_error=0.05,
                             ...
                             drop_k=args.drop_k, max_sample_size=args.max_sample_size)
    ...
    if args.ds_comparison:
        ...
        def retrieve_data_for_ds_comparison(results_path, method, sample_size=None):
            ...
            first_average = np.average(return_data[:, interval_start-10:interval_start], axis=1)
            last_average = np.average(return_data[:, interval_end-10:interval_end], axis=1)
        ...
        retrieve_data_for_ds_comparison(results_path, methods[0], sample_size=250)
        retrieve_data_for_ds_comparison(results_path, methods[1], sample_size=250)