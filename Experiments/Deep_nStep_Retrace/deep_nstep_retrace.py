...
# Replay Buffer
from Experiments_Engine.Function_Approximators import nStep_Retrace_ExperienceReplay
...
from Experiments_Engine.RL_Agents import ExperienceReplay_Agent, nStep_Retrace_ReturnFunction
# Policy
...
class ExperimentAgent:
    ...
    def __init__(self, experiment_parameters, restore=False, restore_data_dir=""):
        ...
        if restore:
            ...
            self.config.target_policy.final_epsilon = self.target_epsilon
            self.config.target_policy.anneal_epsilon = False
            self.config.target_policy.annealing_period = 20000      # Not used
            ...
            self.config.behaviour_policy.annealing_period = 20000   # Not used
        ...
        """ n-Step Retrace Return Function """
        self.rl_return_fun = nStep_Retrace_ReturnFunction(config=self.config, tpolicy=self.target_policy,
                                                          bpolicy=self.behaviour_policy)

        """ n-Step Retrace Replay Buffer """
        self.retrace_erp = nStep_Retrace_ExperienceReplay(config=self.config, return_function=self.rl_return_fun)

        """ Neural Network """
        self.function_approximator = NeuralNetwork_wER_FA(optimizer=self.optimizer, target_network=self.tnetwork,
                                                          update_network=self.unetwork, er_buffer=self.retrace_erp,
                                                          tf_session=self.tf_sess, config=self.config,
                                                          summary=self.summary)

        """ RL Agent """
        self.agent = ExperienceReplay_Agent(function_approximator=self.function_approximator,
                                            behaviour_policy=self.behaviour_policy, environment=self.env,
                                            er_buffer=self.retrace_erp, config=self.config, summary=self.summary)
    ...
    def save_parameters(self, dir_name):
        ...
        params_txt.write("# Agent: n-Step Retrace #\n")
...
if __name__ == "__main__":
    ...
    experiment = Experiment(results_dir=results_directory, save_agent=args.dump_agent, restore_agent=False,
                            max_number_of_episodes=args.episodes, experiment_parameters=args)
